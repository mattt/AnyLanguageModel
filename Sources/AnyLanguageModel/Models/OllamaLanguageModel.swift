    ...
    public func respond<Content>(
        ...
        while true {
            ...
            if let toolCalls = message.toolCalls, !toolCalls.isEmpty {
                ...
                let resolution = try await resolveOllamaToolCalls(toolCalls, session: session)
                switch resolution {
                case .stop(let calls):
                    if !calls.isEmpty {
                ...
                let task = Task {
                    do {
                        ...
                        while true {
                            ...
                            let resolution = try await resolveOllamaToolCalls(streamedToolCalls, session: session)
                            switch resolution {
                            case .stop(let calls):
                                if !calls.isEmpty {
...
private func resolveOllamaToolCalls(
    _ toolCalls: [OllamaToolCall],
    session: LanguageModelSession
) async throws -> ToolResolutionOutcome {
    let calls = try toolCalls.map { call in
        Transcript.ToolCall(
            id: call.id ?? UUID().uuidString,
            toolName: call.function.name,
            arguments: try toGeneratedContent(call.function.arguments)
        )
    }
